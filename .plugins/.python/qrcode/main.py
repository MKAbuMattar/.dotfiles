...
# -*- coding: utf-8 -*-
#---------------------------------------------------------------------------------------------------
# MKAbuMattar's Profile - QRCode Plugin
#
#
#                             .
#         ..                .''
...
Usage:
    python3 main.py "Hello World" --format png
    python3 main.py "https://github.com" --format svg --save
    python3 main.py --interactive --format svg
"""
...
def show_qrcode_input_message() -> None:
...
def get_text_input(input_text: str | None = None) -> str:
    """
...
def generate_qrcode_png(text: str) -> bytes:
    """
...
def save_qrcode(qrcode_data: bytes | str, output_path: str, file_format: str = "PNG") -> None:
    """
    ...
    try:
        ...
        if file_format.upper() == "SVG":
            assert isinstance(qrcode_data, str)
            out.write_text(qrcode_data, encoding='utf-8')
        else:  # PNG
            assert isinstance(qrcode_data, bytes)
            out.write_bytes(qrcode_data)
  ...
  %(prog)s --test
    ...
    if args.output_path or args.save:
        if args.output_path:
            ...
            output_path = str(qr_dir / filename)
