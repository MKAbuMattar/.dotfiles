...
# -*- coding: utf-8 -*-
#---------------------------------------------------------------------------------------------------
# MKAbuMattar's Profile - WebSearch Plugin
#
#
#                             .
#         ..                .''
...
Usage:
    python3 main.py --engine google --query "Python tutorial"
    python3 main.py -e duckduckgo -q "web development"
...
def build_search_url(engine_name: str, query: str, verbose: bool = False) -> str | None:
    """
...
def display_search_url(url: str, engine_name: str) -> None:
    """
...
def open_in_browser(url: str) -> bool:
    ...
    try:
        ...
        print(f"{Colors.RED}[-]{Colors.RESET} Could not open browser: {e}")
        return False


def test_search_service() -> bool:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Search the web in your default browser via 25+ engines",
        prog="web_search.py",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s -e google -q "Python tutorial"              # Opens in browser
  %(prog)s -e ddg -q "web development"                 # Same, via alias
  %(prog)s -e github -q "claude code" -p               # Print URL, don't open
...
Aliases: ddg→duckduckgo, sp→startpage, so→stackoverflow, docker→dockerhub,
         yt→youtube, ppai→perplexity, pip→pypi
        """
    )
    ...
    parser.add_argument("-p", "--print", dest="print_only", action="store_true",
                        help="Print the URL only; do not open the browser")
    ...
    if args.test:
        return 0 if test_search_service() else 1
    ...
    # Print-only: bare URL on stdout (no ANSI, no prefix — pipe-safe)
    if args.print_only:
        print(url)
        return 0

    # Default: show the URL and open it in the default browser
    display_search_url(url, args.engine)
    if not open_in_browser(url):
        print(f"{Colors.YELLOW}[!]{Colors.RESET} URL printed above; copy it manually.")
        return 1
    return 0
