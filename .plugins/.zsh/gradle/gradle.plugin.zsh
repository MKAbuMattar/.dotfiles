...
# Generate gradle completion if it doesn't exist or is older than 7 days
if [[ ! -f "$ZSH_CACHE_DIR/completions/_gradle" ]] || \
   [[ -n "$(find "$ZSH_CACHE_DIR/completions/_gradle" -mtime +7 2>/dev/null)" ]]; then
  gradle --completion=zsh >| "$ZSH_CACHE_DIR/completions/_gradle" 2>/dev/null
fi
